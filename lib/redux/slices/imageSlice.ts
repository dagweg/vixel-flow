...
const imageSlice = createSlice({
  ...
  reducers: {
    ...
    setOriginalImage: (state, action: PayloadAction<OriginalImageState>) => {
      state.originalImage = action.payload;
    },
    setImageSelected: (state, action: PayloadAction<SetImageSelectedState>) => {
      let foundImageIndex = -1;

      for (let i = 0; i < (state.recentModifications?.length! ?? 0); i++) {
        if (
          state.recentModifications?.[i].imageHash === action.payload.imageHash
        ) {
          foundImageIndex = i;
          break;
        }
    ...
    setImageIsBeingProcessed: (state, action: PayloadAction<boolean>) => {
      state.isBeingProcessed = action.payload;
    },
  },
});
